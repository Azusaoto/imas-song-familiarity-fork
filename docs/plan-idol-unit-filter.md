# 計畫書：偶像 + 組合連動篩選器（**已完成 / 歷史文件**）

> **狀態：已完成（commit 98aef69）**
> **後續延伸**：依熟悉度過濾、共同歌單頁改寫（commit f2a669b、fc262f9）已超出本計畫書原始範疇
> 目標版本：v0.2.0 ✅
> 實作工時：合併於本 PR

本文件保留作為當時設計討論的歷史紀錄，**請勿當作未完成的待辦清單**。
篩選器最新行為以程式碼為準（`lib/filterSongs.ts` + `lib/useBrandFilter.ts` + `lib/brandMap.ts`）。
